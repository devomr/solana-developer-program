...
describe("favorites", () => {
  ...
  before(async () => {
    ...
    const balanaceInSOL = balance / web3.LAMPORTS_PER_SOL;
    const formattedBallance = new Intl.NumberFormat().format(balanaceInSOL);
    console.log(`User balance: ${formattedBallance} SOL`);
  });
  ...
  it("Saves as user's favorites to the blockchain", async() => {
    ...
    const favoriteHobbies = ["football", "biking"];
    ...
    const favoritePda = favoritesPdaAndBump[0];
  ...
  it("Doesn't let people write to favorites for other users", async () => {
    ...
    try {
      await program.methods.setFavorites(new anchor.BN(420), "blue", ["basketball"]).signers([someRandomGuy]).rpc();
    } catch (error) {
