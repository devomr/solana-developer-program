...
describe('escrow', () => {
  ...
  before(
    ...
    async () => {
      ...
      // Airdrops to users and creates 2 tokens mint "A" and "B"
      ...
      await provider.sendAndConfirm(tx, [alice, bob, tokenMintA, tokenMintB]);
  ...
  const tokenAOfferedAmmount = new BN(1_000_000);
  const tokenBWantedAmmount = new BN(1_000_000);
  ...
  const make = async () => {
    ...
    const transactionSignature = await program.methods
      .makeOffer(offerId, tokenAOfferedAmmount, tokenBWantedAmmount)
      .accounts({ ...accounts })
      .signers([alice])
      .rpc();
    ...
    assert(vaultBalance.eq(tokenAOfferedAmmount));
    ...
    assert(offerAccount.tokenBWantedAmount.eq(tokenBWantedAmmount));
  };
  ...
  const take = async () => {
    ...
    assert(bobTokenAccountBalanceAfter.eq(tokenAOfferedAmmount));
    ...
    assert(aliceTokenAccountBalanceAfter.eq(tokenBWantedAmmount));
  };
